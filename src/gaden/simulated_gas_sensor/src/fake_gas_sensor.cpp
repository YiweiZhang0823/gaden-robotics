...
void FakeGasSensor::run()
{
    ...
    // wait_for_service 期间未 spin，TF 缓冲为空；暖机让 /tf_static(map→base) 与 robot_state_publisher 的臂链路进 buffer
    {
    ...
    while (rclcpp::ok())
    {
        // 先处理回调，否则第一次 lookup 时缓冲区尚无 /tf_static，会误报 target_frame 不存在
        rclcpp::spin_some(shared_this);
        ...
        {
            // 启动初 map / tcp 未就绪时常态，节流为 WARN，避免刷 ERROR
            RCLCPP_WARN_THROTTLE(
                get_logger(), *get_clock(), 3000,
                "TF lookup not ready: %s", ex.what());
            know_sensor_pose = false;
            using namespace std::literals::chrono_literals;
            rclcpp::sleep_for(std::chrono::duration_cast<std::chrono::nanoseconds>(200ms));
        }
